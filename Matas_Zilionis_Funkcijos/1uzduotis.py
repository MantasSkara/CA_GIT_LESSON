...
def info_apie_sakini(string):

    words = len(string.split())

    upper_counter = sum(thing.isupper() for thing in string)

    lower_counter = sum(thing.islower() for thing in string)

    number_counter = sum(thing.isdigit() for thing in string)
    
    print(f"Words: {words}, didžiosios: {upper_counter}, mažosios: {lower_counter}, skaičiai: {number_counter}")
